...
public class CellComponent extends JComponent {
	 ...
	 * 
	 * @param x      x coordinate of the upper left corner of the enclosing
	 *               rectangle
	 * @param y      y coordinate of the upper left cornet of the enclosing
	 *               rectangle
	 * @param width  width of the enclosing rectangle
	 * @param height height of the enclosing rectange
	 */
	@Override
	 ...
	 * 
	 * @param e MouseEvent object representing mouse's click
	 */
	@Override
	 ...
	 * 
	 * @param g Graphics object used to draw the cells on the screen
	 */
	@Override
	protected void paintComponent(Graphics g) {
		...
		/*
		 * Each map cell has a background color and some of them have an icon in them.
		 * Background color and icon are selected below
		 */
		if (c == CellColors.BLOCK) {
			...
			if (c == CellColors.CUSTOMER_PROCESSED || c == CellColors.INITIAL_PROCESSED)
				gp = new GradientPaint(hexX[nPoints - 1], hexY[nPoints - 1],
						palette.initialGradient(CellColors.IN_STACK), hexX[1], hexY[1],
						palette.gradientColor(CellColors.IN_STACK), true);
			else
				gp = new GradientPaint(hexX[nPoints - 1], hexY[nPoints - 1], palette.initialGradient(c), hexX[1],
						hexY[1], palette.gradientColor(c), true);
			g2d.setPaint(gp);
			g2d.fillPolygon(hexagon);
			if (c == CellColors.CUSTOMER_PROCESSED) {
				...
				g2d.drawImage(img, width / scale, height / scale, (scale - 2) * width / scale,
						(scale - 2) * height / scale, null);
			}
			displayed = true;
			...
			displayed = true;
		} else if (c == CellColors.RIGHT_ROAD || c == CellColors.RIGHT_ROAD_PUSHED
				|| c == CellColors.RIGHT_ROAD_POPPED) {
			...
			g2d.drawImage(img, 0, 0, width, width, null);
			displayed = true;
		} else if (c == CellColors.UP_ROAD || c == CellColors.UP_ROAD_PUSHED || c == CellColors.UP_ROAD_POPPED) {
			...
			g2d.drawImage(img, 0, 0, width, width, null);
			displayed = true;
		} else if (c == CellColors.DOWN_ROAD || c == CellColors.DOWN_ROAD_PUSHED || c == CellColors.DOWN_ROAD_POPPED) {
			// High altitude cells: initial state, when they are visited for the
			// first time,
			// and when the program decides not to include them in the path
			Image img;
			...
			g2d.drawImage(img, 0, 0, width, width, null);
			displayed = true;
		} else if (c == CellColors.CROSSING || c == CellColors.CROSSING_PUSHED || c == CellColors.CROSSING_POPPED) {
			// High altitude cells: initial state, when they are visited for the
			// first time,
			// and when the program decides not to include them in the path
			Image img;
